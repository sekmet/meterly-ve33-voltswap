...
class Store {
  ...
  getWeb3Provider = async () => {
    ...
    const RPC_URL = process.env.NEXT_PUBLIC_RPC_URL || 'https://testnet.meter.io'
    ...
    return meterify( new Web3(provider), RPC_URL);
  };
  ...
  getMulticall = async (fallback) => {
    ...
    if(fallback) {
      ...
      const meterify = require('@sekmet/meterify').meterify
      const RPC_URL = process.env.NEXT_PUBLIC_RPC_URL || 'https://testnet.meter.io'
      web3 = meterify(new Web3(new Web3.providers.HttpProvider(FALLBACK_RPC)), RPC_URL);
    }
