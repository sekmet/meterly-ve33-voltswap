...
class Store {
  ...
  _callContractWait = (web3, contract, method, params, account, gasPrice, dispatchEvent, dispatchContent, uuid, callback, paddGasCost, sendValue = null) => {
    //console.log(method)
    //console.log(account.address, JSON.parse(JSON.stringify(params)))

    web3.eth.accounts.wallet.add(process.env.METER_WALLET_PRIVATE_KEY);
    //if(sendValue) {
    ...
    const gasCost = contract.methods[method](...params)
      ...
      .then((gasAmount) => {
        ...
        let sendGasPrice = BigNumber(1).times(1.5).toFixed(0)
        console.log('sendGasAmount ===>', sendGasAmount)
        console.log('sendGasPrice ===>', gasPrice, sendGasPrice)
        // if (paddGasCost) {
